...
import { convertToBase64 } from "@/utils/imageUtils"; // Added import
...
const Scores = () => {
  ...
  useEffect(() => {
    axios
      .get<SessionData>("/api/auth/get-session")
      .then((res) => {
        setSession(res.data);
      })
      .catch((err) => console.error("❌ Session fetch error:", err));
  ...
  useEffect(() => {
    if (formData.department && formData.course_category) {
      axios
        .get(
          `/api/courses/get-course-templates?department=${encodeURIComponent(
            formData.department
          )}&category=${encodeURIComponent(formData.course_category)}`
        )
        .then((res) => {
  ...
  useEffect(() => {
    const selectedCourse = courseCodes.find(
      (course) => course.course_code === formData.course_code
    );
    if (selectedCourse) {
      setFormData((prev) => ({
        ...prev,
            ...
            {session ? (
              <form onSubmit={handleSubmit} className={styles.formDibox}>
                ...
                <input
                  ...
                  required
                  readOnly={!!formData.teacher_name}
                />
                ...
                <div className={styles.DiBUTTONdiv}>
                  ...
                  <button type="submit" className={styles.DiBUTTON}>
                    Submit
                  </button>
                </div>
              </form>
